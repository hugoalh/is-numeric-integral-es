...
function getNumericIntegralTypeRange(typeName: NumericIntegralType | keyof typeof NumericIntegralType): NumericIntegralTypeRange {
	switch (NumericIntegralType[typeName]) {
		case "int8":
		...
		default:
			throw new RangeError(`\`${typeName}\` is not a valid integral numeric type! Only accept these values: ${Array.from(new Set(Object.keys(NumericIntegralType).sort()).values()).join(", ")}`);
	}
}
/**
 ...
 * ```
 */
export function isNumericIntegral(typeName: NumericIntegralType | keyof typeof NumericIntegralType, item: bigint | number): boolean {
	const {
		maximum,
		minimum
